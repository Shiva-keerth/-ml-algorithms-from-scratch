...
# ── Page config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="SmartFind – AI Product Search",
    page_icon="🔍",
    layout="wide",
...
.nl-hint {
    ...
    border-radius: 8px; padding: 6px 12px; margin-top: 10px;
...
.score-pill {
    font-size: 0.65rem; padding: 2px 6px; border-radius: 99px; font-weight: 500;
}
...
.similar-chip {
    ...
    border-radius: 8px; padding: 4px 10px; margin: 3px; font-size: 0.78rem; color: #374151;
    cursor: pointer;
}
...
@st.cache_resource
def load_all():
    df = pd.read_csv("real product2.csv")
    df.columns = [c.strip().lower().replace(" ", "_") for c in df.columns]

    # Ensure core columns exist before anything else
    ...
    # Optional enrichment columns — safe defaults
    ...
    # Rich search text: name (3x) + brand + category + sub_category + description snippet
    df["search_text"] = (
    ...
    # LSA semantic embeddings (TruncatedSVD on TF-IDF — lightweight semantic search)
    svd = TruncatedSVD(n_components=50, random_state=42)
    lsa_mat = svd.fit_transform(tfidf_mat)

    # KMeans clustering on LSA embeddings
    km = KMeans(n_clusters=8, random_state=42, n_init=10)
    df["cluster"] = km.fit_predict(lsa_mat)

    # Item-item similarity matrix (top-10 per item for recommendations)
    ...
    # Popularity score: use real rating if available, else price-normalized proxy
    df["rating_filled"] = df["rating"].fillna(df.groupby("category")["rating"].transform("median")).fillna(3.0)
...
def parse_nl_filters(query):
    """
    Week 3 unique feature — Natural Language Filter Parser.
    Extracts price ceiling, category, and brand from plain English queries.
    Returns (clean_query, detected_filters_dict).
    """
    filters = {}
    original = query

    # Price: "under 50000", "below ₹60k", "less than 30,000"
    price_pat = re.search(
    ...
    cat_map = {
        ...
        "phone": "Electronics", "mobile": "Electronics", "iphone": "Electronics",
        "samsung": "Electronics", "headphone": "Electronics", "tv": "Electronics",
        "shoe": "Footwear", "sneaker": "Footwear", "boot": "Footwear", "sandal": "Footwear",
        "shirt": "Clothing", "dress": "Clothing", "jeans": "Clothing",
        "trouser": "Clothing", "jacket": "Clothing", "cloth": "Clothing",
        "sofa": "Furniture", "chair": "Furniture", "table": "Furniture",
        "bed": "Furniture", "furniture": "Furniture",
        "washing": "Home Appliances", "fridge": "Home Appliances",
        "refrigerator": "Home Appliances", "appliance": "Home Appliances",
...
def explain_match(query_lower, doc_idx, top_n=5):
    """
    Week 3 unique feature — Why was this recommended?
    Returns top contributing TF-IDF terms.
    """
    qvec = vectorizer.transform([query_lower]).toarray()[0]
    dvec = tfidf_mat[doc_idx].toarray()[0]
    contrib = qvec * dvec
    top_idx = contrib.argsort()[::-1][:top_n]
    return [(FEAT_NAMES[i], round(float(contrib[i]), 4))
            for i in top_idx if contrib[i] > 0]
...
def hybrid_search(query, alpha, threshold, cat_filter, price_min, price_max,
                  sort_by, use_rerank):
    """
    Week 2 core — Hybrid TF-IDF + LSA semantic search with optional popularity re-ranking.
    alpha = 0 → pure semantic | alpha = 1 → pure TF-IDF
    """
    ...
    fuzzy_hits = fuzz_process.extract(
        query, df["search_text"].tolist(), limit=5, score_cutoff=40
    )
...
def ab_compare(query, threshold=0.1):
    """
    Week 3 unique feature — A/B test: TF-IDF only vs Hybrid search.
    Returns both result sets and comparison metrics.
    """
...
with tab_search:
    ...
    if search_btn and query_raw.strip():
        ...
        # Merge NL filters with sidebar values
        ...
        if results.empty:
            ...
            <div class="no-results">
                ...
                <p>Lower the match threshold or try broader terms.</p>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="result-header">
                <div class="result-count">
                    <strong>{len(results)}</strong> results for <strong>"{query_raw}"</strong>
            ...
            for rank, (_, row) in enumerate(results.iterrows(), 1):
                ...
                with cols[col_idx]:
                    ...
                    colour     = bar_color(float(row["similarity"]))
                    rank_cls   = f"rank-{rank}" if rank <= 3 else "rank-n"
                    img_url    = (row.get("image_url") or "").strip() or get_product_image(row["product_name"], row["category"])
                    ...
                    try:
                        ...
                        disc_pct = 0
                    disc_html = f'<span class="disc-badge">{disc_pct}% off</span>' if disc_pct >= 5 else ""

                    # Star rating
                    try:
                        rating_val = float(row["rating"])
                        stars = "★" * int(round(rating_val)) + "☆" * (5 - int(round(rating_val)))
                        rating_html = f'<div class="card-rating"><span class="stars">{stars}</span> <span class="rating-num">{rating_val}</span></div>'
                    except:
                        rating_html = ""

                    # Brand
                    brand = str(row.get("brand", "")).strip()
                    brand_html = f'<div class="card-brand">{brand}</div>' if brand and brand != "Unknown" else ""
                    ...
                    <div class="product-card">
                        ...
                        <img class="card-img"
                             src="{img_url}"
                             onerror="this.style.background='#f3f4f6';this.style.height='140px';this.removeAttribute('src')"
                             loading="lazy"
                             alt="{row['product_name']}"/>
                        <div class="card-body">
                            <div class="card-category {cat_css(row['category'])}">{row['category']}</div>
                            {brand_html}
                            <div class="product-name">{row['product_name'][:60]}{'...' if len(str(row['product_name'])) > 60 else ''}</div>
                            {rating_html}
                            <div class="card-row">
                                <div>
                                    <div class="product-price">₹{int(row['price']):,}</div>
                                    <div class="pred-price">AI est: ₹{int(row['predicted_price']):,}</div>
                                </div>
                                <div class="cluster-badge">{CLUSTER_NAMES.get(int(row['cluster']),'Group')}</div>
                            </div>
                            <div class="match-label">
                                <span>Match</span><span>{score_pct}%</span>
                            </div>
                            <div class="match-bar-bg">
                                <div class="match-bar-fill"
                                     style="width:{score_pct}%;background:{colour}"></div>
                            </div>
                            <div class="score-breakdown">
                                <span class="score-pill score-tfidf">TF-IDF {row['tfidf_score']}%</span>
                                <span class="score-pill score-semantic">LSA {row['semantic_score']}%</span>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if show_similar:
                        ...
                        if similar:
                            chips = "".join(
                                f'<span class="similar-chip">🔗 {s["product_name"][:22]} · ₹{int(s["price"]):,}</span>'
                                for s in similar
                            )
                            st.markdown(
                                f'<div class="similar-header">Similar products</div>{chips}',
                                unsafe_allow_html=True
                            )
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 2 — A/B TEST (Week 3 unique feature)
# ══════════════════════════════════════════════════════════════════════════════
with tab_ab:
    st.subheader("A/B test: TF-IDF only vs Hybrid search")
    st.caption(
        "This panel runs the same query through two ranking algorithms side-by-side "
        "and measures result quality — exactly what data scientists do at e-commerce companies."
    )

    ab_query = st.text_input("Enter a query to test", placeholder="e.g. running shoes",
                             key="ab_query")
    ...
    if ab_btn and ab_query.strip():
        ...
        st.info(
            "**Interview talking point:** This is called offline evaluation of ranking algorithms. "
            "In industry, data scientists use metrics like Precision@K, NDCG, and MRR to decide "
            "which search algorithm to deploy. Higher avg score + more results = better algorithm."
        )
    elif ab_btn:
...
with tab_analytics:
    ...
    if log:
        ...
        with col_t:
            ...
            st.markdown(f'<div style="background:#fff;border:1px solid #e8e8f0;border-radius:12px;padding:1rem">{rows}</div>',
                        unsafe_allow_html=True)
        with col_h:
