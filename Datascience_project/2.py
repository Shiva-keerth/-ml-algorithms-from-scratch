...
with st.sidebar:
    selected = option_menu(
        ...
        menu_icon="briefcase",
        default_index=1  # Opening to Add Employee for testing
    )
...
# --- TAB 2: ADD NEW EMPLOYEE (WITH LONGITUDINAL DATA) ---
elif selected == "Add New Employee":
    st.title("Add New Employee")
    st.markdown(
        "Enter the static technical scores below, then use the dynamic table to add tracking data over multiple days or weeks. The AI will analyze the trends.")

    # Auto-generate next Employee ID
    next_id_num = len(st.session_state.df) + 1
    default_emp_id = f"EMP_{str(next_id_num).zfill(5)}"

    # 1. New Basic Info Fields
    ...
    with col_d:
        emp_dept = st.text_input("Department", "Data Analytics")

    st.divider()

    # 2. Static Technical Scores
    ...
    # 3. Dynamic Longitudinal Data (Multiple Days/Months)
    st.subheader("Longitudinal Tracking (Behavioral Metrics)")
    st.write(
        "Add a new row for each day, week, or month. The model will calculate the average to predict overall performance.")

    # Default starter data for the table
    ...
    # The dynamic editor that allows adding multiple rows
    edited_tracking_df = st.data_editor(default_tracking_data, num_rows="dynamic", use_container_width=True)

    submitted = st.button("Save Employee & Run ML Prediction", type="primary")

    if submitted:
        # Check if they entered at least one row of tracking data
        if edited_tracking_df.empty:
            st.error("Please add at least one row of tracking data.")
        else:
            with st.spinner("Aggregating Data and Training Random Forest..."):
                # Calculate the averages from the multiple inputs
                ...
                # --- YOUR EXACT ML STRUCTURE ---
                ml_df = pd.read_csv(FILE_NAME)
                le = LabelEncoder()
                ...
                # Prediction using the aggregated averages
                newdata = [[
            ...
            # Save the new row (including the new Name and Email fields)
            new_row = {
                'Employee_ID': emp_id,
                'Employee_Name': emp_name,  # New Field
                'Employee_Email': emp_email,  # New Field
                'Department': emp_dept,  # New Field mapped
                ...
                'Performance_Trend': predicted_trend,
                'Attrition_Risk_Level': 'Medium'
            }
            ...
            st.success(f"Employee {emp_name} ({emp_id}) added successfully with aggregated tracking data!")
            st.info(f"AI Predicted Performance Trend based on aggregated inputs: **{predicted_trend}**")
...
# --- TAB 4: REMOVE EMPLOYEE (NEW) ---
    ...
    # Show a quick preview so HR knows who they are deleting
    if emp_to_remove:
        ...
        if st.button("Permanently Delete Employee", type="primary"):
            # Filter the dataframe to KEEP everyone EXCEPT the selected employee
...
# --- TAB 5: AI MODEL INSIGHTS ---
    ...
    st.write(le.inverse_transform(prediction[:10]))  # Just showing first 10 so it doesn't flood the screen
